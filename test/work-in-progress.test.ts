...
describe.skip('Other tests', () => {
  ...
  it('should redirect to "checkout.stripe.com" after clicking the "Checkout" button', async () => {
    ...
    await Promise.all([checkoutButton.click(), page.waitForNavigation({ waitUntil: 'networkidle0' })])
  ...
  it('should complete Stripe checkout form and redirect to account page', async () => {
    ...
    await Promise.all([submitButton.click(), page.waitForNavigation({ waitUntil: 'networkidle0' })])
    const currentUrl = page.url()
  ...
  it(`shows the purchased book in the "Purchase history" table with 5 downloads remaining`, async () => {
    ...
    const downloadButtonText = await downloadButton.evaluate(element => element.textContent)
    expect(downloadButtonText).toBe('Download')

    const downloadsRemaining = await getByTestId('dracula-downloads-remaining')
    const downloadsRemainingText = await downloadsRemaining.evaluate(element => element.textContent)
