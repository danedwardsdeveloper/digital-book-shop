...
import { page, baseURL } from './setup.test';
...
export async function continueShopping() {
	...
	const buttonText = await continueShoppingButton.evaluate(
...
export async function clickNavLink(dataTestID: string, path: string = '/') {
	...
	const expectedPath = path.startsWith('/')
	...
	if (currentUrl !== expectedPath) {
		throw new Error(
			`Expected URL to be ${expectedPath}, but got ${currentUrl}`
		);
	}
}
