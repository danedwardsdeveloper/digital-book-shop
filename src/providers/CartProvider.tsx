...
import { type CartItem, MongoCartItem, type UserType } from '@/types'
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addToCart = async (slug: string) => {
    setCart(prevCart => {
      ...
      if (existingItem) {
        const newCart = prevCart.map(item => (item.slug === slug ? { ...item, removed: false } : item))
  ...
  const removeFromCart = async (slug: string) => {
    setCart(prevCart => {
      const newCart = prevCart.map(item => (item.slug === slug ? { ...item, removed: true } : item))
      updateLocalStorage(newCart)
      return newCart
    })
  }
  ...
  const toggleCartItem = async (slug: string) => {
    setCart(prevCart => {
      ...
      let newCart
      if (existingItem) {
        newCart = prevCart.map(item => (item.slug === slug ? { ...item, removed: !item.removed } : item))
      } else {
  ...
  const mergeLocalAndDatabaseCarts = async (user: UserType) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error('Failed to sync cart with database:', response.status)
      }
    } catch (error) {
